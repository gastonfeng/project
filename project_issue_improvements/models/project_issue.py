...
class ProjectIssueImprovements(models.Model):
    ...
    sale_subscription_id = fields.Many2one(comodel_name='sale.subscription', string="Subscription",
                                           compute='_compute_sale_subscription')
    ...
    def convert_to_task(self):
        for issue in self:
            ...
            project_task_type_converted = project_task_type_obj.search(self.env.cr, self.env.uid,
                                                                       [('name', '=', 'Converted to Task')])
            project_task_type_converted_id = (
                project_task_type_obj.browse(self.env.cr, self.env.uid, project_task_type_converted[0])).id
